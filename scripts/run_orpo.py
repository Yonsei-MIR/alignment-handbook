...
def main():
    ...
    if pre_datasets is None:
        ...
        logger.info(
            f"Training on the following splits: {[split + ' : ' + str(dset.num_rows) for split, dset in pre_datasets.items()]}"
        )
    ...
    for index in random.sample(range(len(pre_datasets["train"])), 3):
        logger.info(f"Prompt sample {index} of the raw training set:\n\n{pre_datasets['train'][index]['prompt']}")
        logger.info(f"Chosen sample {index} of the raw training set:\n\n{pre_datasets['train'][index]['chosen']}")
        logger.info(f"Rejected sample {index} of the raw training set:\n\n{pre_datasets['train'][index]['rejected']}")
    ...
    trainer = ORPOTrainer(
        ...
        eval_dataset=pre_datasets["test"] if "test" in pre_datasets else None,
        tokenizer=tokenizer,
    ...
    metrics["train_samples"] = len(pre_datasets["train"])
    ...
    # Evaluate
    ##########
    if training_args.do_eval and "test" in pre_datasets:
        logger.info("*** Evaluate ***")
        metrics = trainer.evaluate()
        metrics["eval_samples"] = len(pre_datasets["test"])
